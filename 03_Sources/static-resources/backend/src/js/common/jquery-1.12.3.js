...
(function(global, factory) {
	...
	(
		...
		function(window, noGlobal) {
			...
			// Object.prototype collisions)
			// throw uncatchable exceptions if you attempt to set
			// expando properties
			noData : {
			    "applet " : true,
			    "embed " : true,
			...
			if (elem.detachEvent) {
			    ...
			    // #8545, #7054, preventing memory leaks for custom
			    // events in IE6-8
			    // detachEvent needed property on element, by name
			    // of that event,
			...
			jQuery.event.special.submit = {
				...
				jQuery.event.add(this, "click._submit keypress._submit", function(e) {
				    ...
				    // to allow fixing the IE8 delegated submit
				    // issue (gh-2332)
			...
			empty : function() {
				...
				// If this is a select, ensure that it displays
				// empty (#12336)
				// Support: IE<9
			...
			style : function(elem, name, value, extra) {
				...
				// Make sure that null and NaN values aren't
				// set. See: #7116
				...
				// (for every problematic property) identical
				// functions
			...
			return str && !jQuery.trim(str.replace(rvalidtokens, function(token, comma, open, close) {
			    ...
			    // Perform no more replacements after returning to
			    // outermost depth
			    if (depth === 0) {
				return token;
			    }
			    ...
			    // true (decrement)
			    // other cases ("," or primitive): depth += true -
			    // true (numeric cast)
			    depth += !close - !open;
		    ...
		    // For all other browsers, use the standard XMLHttpRequest
		    // object
		    createStandardXHR;
			...
			jQuery.ajaxTransport(function(options) {
				...
				return {
					...
					// Do send the request
					// `xhr.send` may raise an exception,
					// but it will be
					// handled in jQuery.ajax (so no
					// try/catch here)
					if (!options.async) {
					    ...
					    // Register the callback, but delay
					    // it in case `xhr.send` throws
					    // Add to the list of active xhr
					    // callbacks
		    ...
		    jQuery.ajaxTransport("script",
				...
				if (s.crossDomain) {
					...
					send : function(_, callback) {
					    ...
					    // Circumvent IE6 bugs with base
					    // elements (#2709 and #4378) by
					    // prepending
					    // Use native DOM manipulation to
					    // avoid our domManip AJAX trickery
			...
			if (self.length > 0) {
				...
				// If the request succeeds, this function gets
				// "data", "status", "jqXHR"
				// but they are ignored because response was set
				// above.
				// If it fails, this function gets "jqXHR",
				// "status", "error"
